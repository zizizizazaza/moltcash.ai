    ...
    for attempt in range(max_retries + 1):  # +1 for initial attempt
        try:
            ...
            if response.status_code == 429 and attempt < max_retries:
                # Reduced backoff to 5s, 10s... to avoid hanging the chat forever
                delay = 5 + (5 * attempt)
            ...
            if attempt < max_retries:
                ...
                print(f"Max retries exceeded for {url}. Returning mock 500 response.")
                mock_resp = requests.Response()
                mock_resp.status_code = 500
                mock_resp._content = b'{"error": "Connection failed or timed out"}'
                return mock_resp
